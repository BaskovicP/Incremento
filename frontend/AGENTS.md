...
- `reviewer_button_style.py` owns shared reviewer button geometry, light/dark action colors, focus/disabled states, and the compact Done separator. Extract and Done injectors must not reintroduce independent gradients, shadows, typography, or sizing; answer-grade colors remain in the topic styling adapter.
...
- `reviewer_topic_actions.py` adds the secondary English **Done** button on both reviewer sides, **More → Revisit in…**, and non-modal Undo feedback. Keep commands scoped to the originating card/profile, capture the configured Done tag before background dispatch, preserve native operation-driven reviewer advancement, and leave More/Same/Less answer semantics unchanged. Done suspends only the topic card and adds a note-level tag shared by its siblings; sibling/descendant schedules remain unchanged. Keep **Settings → Topics → Done tag**, default `topic/done`, root config wiring, and one-step suspension/tag Undo aligned.
...
- PDF, EPUB, and video Review All is separate from the normal session builder. Keep Topic/Item, direct/nested, entire/up-to-current, due-only, limit, position-order, preview, and unavailable-card wording aligned. **Reminder without schedule changes** is an opt-in Anki Preview mode; remember it per profile/media type during the current run, keep it independent of card selection, and show the selected scheduling behavior before launch. Run the initial inspection with `QueryOp`, re-resolve and build with `CollectionOp`, and never scan or mutate a potentially large linked-card deck on Qt's main thread. Including cards from other filtered decks must default off, never be remembered, explain that every conflicting deck is emptied, and leave an active reviewer before scheduling the collection mutation.
