...
- Large-session regressions must exercise a 9,999-card target with scarce/exhausted pools, bounded misses, cached candidate searches, one priority sort per pool across incremental auto-refill transactions, rollback-safe cursor rewinding, compact filtered-deck searches, and batch order updates. Endpoint ratios must prove that 0%-weight buckets are never selected.
