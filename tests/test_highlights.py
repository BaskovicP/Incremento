...
class TestLoadHighlights:
    def test_empty_when_none_added(self, tmp_path):
        assert load_highlights(str(tmp_path), 1) == []

    def test_returns_added_highlight(self, tmp_path):
        hl = make_hl()
        add_highlight(str(tmp_path), 1, hl)
        result = load_highlights(str(tmp_path), 1)
        assert len(result) == 1
    ...
    def test_only_returns_highlights_for_card(self, tmp_path):
        add_highlight(str(tmp_path), 1, make_hl("hl-a"))
        add_highlight(str(tmp_path), 2, make_hl("hl-b"))
        result1 = load_highlights(str(tmp_path), 1)
        result2 = load_highlights(str(tmp_path), 2)
...
class TestAddHighlight:
    def test_adds_multiple_highlights(self, tmp_path):
        add_highlight(str(tmp_path), 1, make_hl("hl-1"))
        add_highlight(str(tmp_path), 1, make_hl("hl-2", page=2, color="blue"))
        result = load_highlights(str(tmp_path), 1)
        assert len(result) == 2

    def test_replace_on_duplicate_id(self, tmp_path):
        add_highlight(str(tmp_path), 1, make_hl("hl-1", text="old"))
        add_highlight(str(tmp_path), 1, make_hl("hl-1", text="new"))
        result = load_highlights(str(tmp_path), 1)
        assert len(result) == 1
        assert result[0]["text"] == "new"

    def test_defaults_for_missing_fields(self, tmp_path):
        add_highlight(str(tmp_path), 5, {"id": "hl-x"})
        result = load_highlights(str(tmp_path), 5)
        assert result[0]["page"] == 1
        assert result[0]["color"] == "yellow"
...
class TestRemoveHighlight:
    def test_removes_specific_highlight(self, tmp_path):
        add_highlight(str(tmp_path), 1, make_hl("hl-1"))
        add_highlight(str(tmp_path), 1, make_hl("hl-2"))
        remove_highlight(str(tmp_path), 1, "hl-1")
        result = load_highlights(str(tmp_path), 1)
        assert len(result) == 1
    ...
    def test_remove_nonexistent_is_noop(self, tmp_path):
        add_highlight(str(tmp_path), 1, make_hl("hl-1"))
        remove_highlight(str(tmp_path), 1, "nonexistent")
        assert len(load_highlights(str(tmp_path), 1)) == 1

    def test_does_not_remove_other_cards_highlight(self, tmp_path):
        add_highlight(str(tmp_path), 1, make_hl("hl-1"))
        add_highlight(str(tmp_path), 2, make_hl("hl-1"))
        remove_highlight(str(tmp_path), 1, "hl-1")
        assert load_highlights(str(tmp_path), 1) == []
        assert len(load_highlights(str(tmp_path), 2)) == 1

    def test_remove_all_leaves_empty(self, tmp_path):
        add_highlight(str(tmp_path), 3, make_hl("hl-a"))
        remove_highlight(str(tmp_path), 3, "hl-a")
        assert load_highlights(str(tmp_path), 3) == []
