(function() {
  ...
  function usePdfRender() {
    ...
    const cardIdRef = reactExports.useRef(null);
    const filenameRef = reactExports.useRef(null);
    const pageRef = reactExports.useRef(1);
    const zoomRef = reactExports.useRef(1);
    const lastRenderWidthRef = reactExports.useRef(0);
    const lastScaleRef = reactExports.useRef(1);
    const readPageRef = reactExports.useRef(0);
    const canvasARef = reactExports.useRef(null);
    ...
    const renderTextLayer = reactExports.useCallback((pg, viewport) => {
      var _a;
      ...
      const wrapperW = ((_a = containerRef.current) == null ? void 0 : _a.offsetWidth) ?? viewport.width;
    ...
    const renderPage = reactExports.useCallback((num) => {
      ...
      doc.getPage(num).then((pg) => {
        ...
        const colW = ((_a2 = containerRef.current) == null ? void 0 : _a2.offsetWidth) || 800;
    ...
    const nav2 = reactExports.useCallback((delta) => {
      const doc = pdfDocRef.current;
      if (!doc) return;
    ...
    const adjustZoom = reactExports.useCallback((dir) => {
    ...
    reactExports.useEffect(() => {
      ...
      const observer = new ResizeObserver((entries) => {
        var _a;
        const newW = Math.round(((_a = entries[0]) == null ? void 0 : _a.contentRect.width) ?? 0);
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      ...
      snapshotMode && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "div",
        {
          ...
          children: snapRect && snapRect.w > 2 && snapRect.h > 2 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: {
            position: "absolute",
            left: snapRect.x,
            top: snapRect.y,
            width: snapRect.w,
  ...
  function isSelectionInside(sel, container2) {
    ...
    for (let i = 0; i < sel.rangeCount; i += 1) {
      ...
      if (!container2.contains(node)) return false;
    }
    return true;
  }
  ...
  function findExcerptSpanMatch(textLayer, excerpt) {
    ...
    const spans = Array.from(textLayer.querySelectorAll("span")).map((span) => ({ span, text: normalizeJumpText(span.textContent) })).filter(({ text }) => text.length > 0);
    if (!spans.length) return null;
    const maxWindow = 48;
    ...
    return spans.filter(({ text }) => tokens.some((token) => text.includes(token))).slice(0, Math.max(1, Math.min(tokens.length, 4))).map(({ span }) => span);
  }
  function PdfViewer() {
    const {
      page,
    ...
    const limitTotal = Number((limitStatus == null ? void 0 : limitStatus.daily_page_limit) || 0);
    const limitRemaining = Number((limitStatus == null ? void 0 : limitStatus.pages_remaining) || 0);
    ...
    reactExports.useEffect(() => {
      ...
      if (pendingId) {
        ...
        if (!target || target.page !== page || !((_a = target.rects) == null ? void 0 : _a.length)) return;
      ...
      if (jumpExcerpt) {
        ...
        if (!normalizedExcerpt) {
          ...
          const highlightTarget = highlights.find((highlight) => {
            var _a2;
            if (highlight.page !== page || !((_a2 = highlight.rects) == null ? void 0 : _a2.length)) return false;
            const highlightText = normalizeJumpText(highlight.text);
            return highlightText.includes(normalizedExcerpt) || normalizedExcerpt.includes(highlightText);
          });
          ...
          if (tl && spanCount > 0) {
            ...
            if (matchedSpans == null ? void 0 : matchedSpans.length) {
    ...
    const handleSnapMove = reactExports.useCallback((e) => {
    ...
    const pickHighlightColor = reactExports.useCallback((color, applyNow = false) => {
      ...
      if (makeHighlight(sel, color) && (sel == null ? void 0 : sel.removeAllRanges)) {
        sel.removeAllRanges();
      }
    }, [makeHighlight]);
    ...
    reactExports.useEffect(() => {
      const startWithHighlights = (cardId, filename, startPage, startZoom, startReadPage = 0, startReadAnchor = null, startSearchQuery = "", startJumpExcerpt = "", startScrollToReadAnchor = false, startLimitStatus = null, startAutoHighlightOnExtract = void 0, startBookmarks = null) => {
    ...
    reactExports.useEffect(() => {
      const onCopy = (e) => {
        ...
        if (!cleaned) return;
        (_a = e.clipboardData) == null ? void 0 : _a.setData("text/plain", cleaned);
        e.preventDefault();
      };
                ...
                limitNotice && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "div",
                  {
                    style: {
                      ...
                      padding: "6px 10px",
                      borderRadius: 8,
                      background: limitNotice.kind === "warning" ? "rgba(245,158,11,0.16)" : limitNotice.kind === "info" ? "rgba(59,130,246,0.15)" : "rgba(239,68,68,0.14)",
                      border: limitNotice.kind === "warning" ? "1px solid rgba(245,158,11,0.40)" : limitNotice.kind === "info" ? "1px solid rgba(59,130,246,0.40)" : "1px solid rgba(239,68,68,0.40)",
                      fontSize: 12
                    },
                    children: [
  ...
  if (container) {
    clientExports.createRoot(container).render(/* @__PURE__ */ jsxRuntimeExports.jsx(PdfViewer, {}));
  }
})();
