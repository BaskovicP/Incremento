...
- A scheduler weight of 0 disables that bucket completely; smoothing/epsilon and empty-pool fallback apply only to positive-weight buckets. At a Topic/Item endpoint, report pool exhaustion instead of crossing into the disabled type. Forced card types in strict phases must not fall back and consume another type's quota.
- The tag-quota Other bucket excludes every active selected tag and its subtags, including across topic/item fallback, document/media picks, leftover-slot filling, and auto-refill. Use quoted, grouped Anki exclusions in cache keys as well as searches; return `NO_TAGS_KEY` for Other picks so cumulative quota counts remain accurate. Unrelated tags are allowed.
...
- Session selection is read-only and can be expensive, so run it through a no-progress `QueryOp`; do not wrap the scan in `CollectionOp`, whose application-modal progress window disables the deck browser. The bounded initial filtered-deck build uses `_run_collection_mutation_without_progress()`: a serialized `QueryOp` mutation followed by `on_op_finished`, so normal undo/UI change notifications fire without creating a native modal window. Active-session refill and larger explicit-review builds remain `CollectionOp` mutations. Every phase uses the operation-provided `col` rather than `mw.col`, and Qt/UI work stays in success callbacks. Reject stale results after a profile switch or newer launch. Enter review only after collection hooks and native modal teardown settle; do not gate activation on `ProgressManager.single_shot()`, which can wait forever on an unrelated nested progress level.
...
- `backend/operation_journal.py` owns imports spanning Anki, SQLite, and profile files. Journal each unique profile-relative output before the side effect, keep stable content identity in SQLite, bind Anki identities, then commit `content_items`. `Incremento_Content_ID` is optional legacy note metadata; do not require or auto-create it.
- Before a card exists, rollback may remove only paths tracked by that operation. After a card exists, preserve the data and let bounded profile-open recovery inspect that pending journal row. `backend/reconciliation.py` may rebind through an optional exact content ID or the existing exact provenance source link. Profile-open recovery must never enumerate all Anki cards/notes or scan Incremento indexes; full reconciliation may delete only rows whose Anki owner is definitely gone and must remain an explicit maintenance operation. Neither path may guess-delete legacy files.
...
- `topic_review_actions.py` owns explicit **Done** (native single-card suspension plus a note tag) and **Revisit in…** (native due-date change). Done uses config-service `topic_done_tag` (default `topic/done`), preserves existing tags, and merges tag addition into the suspension's native Undo step; rollback both on failure. Sibling cards share the note tag but retain their schedules. Revisit's `incrv` Anki custom-data day keeps due-independent reader pools paused; preserve other custom data, merge into one native undo step, and include missing/expired markers. Do not create a second SQLite suspension registry. Notification Undo must reject an intervening action or an already-undone step.
...
- EPUB sanitization may retain local `<a href>` targets and uncredentialed HTTP(S) anchors for the reader's opt-in link bridge, but remote resource URLs, scripts, active attributes, unsupported schemes, and escaping paths remain blocked. Bump the sanitizer version whenever that cached-output contract changes.
...
- `backend/media_review.py` resolves Review All cards for PDF, EPUB, and video sources. It unions legacy positioned PDF/EPUB source rows, exact `Incremento_Parent_Card_ID` matches, saved reader links, recent explicit video card positions, and knowledge-tree descendants. Traverse from both the media card and every direct attachment, because an attached card may be a standalone knowledge-tree root; its children are nested links and inherit its media position. Preserve Topic/Item classification through one resolved classifier per scan; direct/nested depth; inherited media position; Anki due-state search; suspended, buried, missing, and other-filtered-deck diagnostics; post-filter limits; deterministic preview/build random order; and exact selected order in the filtered deck. Review All defaults to rescheduling; its explicit reminder mode passes `reschedule=False` through `start_explicit_review_from_selector()` to the shared deck builder. Keep selection independent of this mode and reset the deck configuration on every build. Preview answers must preserve card schedules, topic state, and custom rules. Reclaiming a selected card from another filtered deck must be explicit and use Anki's supported whole-deck empty operation: return every card in each conflicting deck to its original deck, retain the filtered-deck definition, and only then build the target deck. Never mutate an active reviewer deck underneath the displayed card.
