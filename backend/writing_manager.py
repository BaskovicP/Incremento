...
def writing_note_type_spec() -> NoteTypeSpec:
    return NoteTypeSpec(
        name=WRITING_NOTE_TYPE,
        fields=("Title", WRITING_FILE_FIELD, *INCREMENTO_METADATA_FIELDS),
        question_template=CARD_TEMPLATE_FRONT,
        answer_template=CARD_TEMPLATE_BACK,
    )


def ensure_writing_note_type(col, *, allow_existing_update: bool = False) -> None:
    ensure_note_type(
        col,
        writing_note_type_spec(),
